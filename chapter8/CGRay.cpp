...
namespace CCG
{
	...
	*/
	...
	{
		CCGHitRecord sRec;
		if (pHWorld->Hit(Ray, 0.001, CCGDB_MAX, sRec)) {
			CCGRay RayScattered;
			CCGVec3 vAttenuation;

			if (nDepth < 50 && sRec.matPtr->Scatter(Ray, sRec, vAttenuation, RayScattered)){
				return vAttenuation*Color(RayScattered, pHWorld, nDepth + 1);
			}
			else {
			...
			CCGVec3 vUnitDir = unit_vector(Ray.Direction());
			CCGdouble t = 0.5*(vUnitDir.y() + 1.0);
			return (1.0 - t)*CCGVec3(1.0, 1.0, 1.0) + t*CCGVec3(0.5, 0.7, 1.0);
		}
	}
